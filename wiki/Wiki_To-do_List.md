...
- <s>Check that DFN events email address has been created and
  delegated</s> mt
- <s>create initial GUI page,</s> <s>[gui](Camera_GUI "wikilink") fill
...
- Set up VM for offline access using [Kiwix](http://www.kiwix.org/) by
  installing the tools necessary to create a Zim file:
  <http://www.openzim.org/wiki/Build_your_ZIM_file>
  <https://github.com/openzim/mwoffliner>
  <https://github.com/openzim/zimwriterfs> and testing the process.
- <s>Page on how the Nikon D800/E/D810 DSLR's should be configured
  (starting from a full config reset)</s>
- <s>Page on how to set up the Commell bios from scratch (at least for
  EXT's)</s>
- Fix the short URL problem (whoever fixes this will get a prize from
  Robert)
...
- <s>setup dfnstation.cfg file [Configuring the
  Software](Configuring_the_Software "wikilink")</s> mt
- check that [:<File:Good> Quality
  Image.jpg](:File:Good_Quality_Image.jpg "wikilink") is the best image
  possible
...
- how to apply [nanocoat to lenses](How_to_Apply_Nanocoating "wikilink")
  (with pictures or video) zacc [Camera
  Installation](Camera_Installation "wikilink")
- <s>what type of camera do I have?</s> mc
- <s>hosts file page (copy and paste sections) or append bash
  script</s> hard
- <s>setting up your computer (to make things easy) incorporating above
  hosts file</s> hard
- <s>how to generate, install and use a key (including persistent
  agent)</s> rh
- <s>make a camera formatting gif (two button method, filesize under
  8mb)</s>
...
- <s>Page on local log in with a monitor and keyboard to be created </s>
  [eks](Manual_Maintenance "wikilink")
- find nice dfn pictures
  - <s>outback stand install gallery (ask Robert how to do a gallery or
    look here: [Rooftop Fireball Camera
    Installation](Rooftop_Fireball_Camera_Installation "wikilink"))</s>
  - [gallery of fireballs](gallery_of_fireballs "wikilink"), planes,
...
- link squad - morgs & patrick
  [Special:AllPages](Special:AllPages "wikilink")
  [Special:OrphanedPages](Special:OrphanedPages "wikilink")